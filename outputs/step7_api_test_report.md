...
| `session.io.load_default_data() wrapper` | PASS | 0.2 | um_per_px=0.24 |
...
| `preprocessing.binarization.otsu()` | PASS | 0.4 | ['original_video', 'grayscale_video', 'binary_video'] dtype=uint8 uniq=[0, 255] |
...
| `preprocessing.binarization.sauvola()` | PASS | 0.9 | ['original_video', 'grayscale_video', 'binary_video'] dtype=uint8 uniq=[0, 255] |
| `preprocessing.binarization.urbano()` | PASS | 4.3 | ['original_video', 'grayscale_video', 'binary_video'] dtype=uint8 uniq=[0, 1] |
| `binarization.otsu() grayscale yapilmadan (ham renkli video)` | PASS | 0.6 | ['original_video', 'binary_video'] |
...
| Test | Sonuc | Sure (s) | Not |
|---|---|---|---|
| `detection.yolo() varsayilan (yolo26) + evaluate_detections` | PASS | 4.6 | tp=2101 fp=1221 fn=26 P=63.25 R=98.78 F1=77.12 frames=21 |
| `detection.yolo(weights=<ASCII tam yol>) ozel agirlik` | PASS | 2.4 | tp=2101 fp=1221 fn=26 P=63.25 R=98.78 F1=77.12 frames=21 |
| `detection.yolo(conf=0.5) yuksek esik` | PASS | 2.3 | tp=2068 fp=1141 fn=59 P=64.44 R=97.23 F1=77.51 frames=21 |
...
| `get_detections() satir formati (yolo26, frame 0)` | PASS | 2.4 | [['0', '0.910138', '0.828910', '0.037355', '0.046788'], ['0', '0.556985', '0.449665', '0.036418', '0.045641']] |
...
| `detection.digital_washing() n_jobs=1` | PASS | 15.9 | tp=84 fp=15 fn=17 P=84.85 R=83.17 F1=84.0 frames=1 |
| `detection.digital_washing(n_jobs=4) paralel` | PASS | 17.6 | tp=84 fp=15 fn=17 P=84.85 R=83.17 F1=84.0 frames=1 |
...
| `assessment.evaluate_detections() detection YOKKEN -> hata beklenir` | **FAIL** | 0.2 | beklenen Exception gelmedi, sonuc: <pycasa.casa.casa.Casa object at 0x00000284FFF5CE60> |
| `detection overwrite uyarisi (urbano -> moving_cells)` | PASS | 6.0 | detection frame sayisi=1 |

## detection_yolov5 (3/3)

| Test | Sonuc | Sure (s) | Not |
|---|---|---|---|
| `detection.yolo(yolo_model='yolov5') + evaluate_detections` | PASS | 6.1 | tp=1539 fp=631 fn=588 P=70.92 R=72.36 F1=71.63 frames=21 |
| `detection.yolo(yolov5, conf=0.4)` | PASS | 2.2 | tp=1413 fp=315 fn=714 P=81.77 R=66.43 F1=73.31 frames=21 |
...
| `tracking.sort(skip_gt=True) sadece yolo26` | PASS | 2.8 | backend=sort yolo26:239 track |
...
| `tracking.jpdaf(position_gate=5, velocity_gate_um=20, detection_probability=0.9)` | PASS | 11.1 | backend=jpdaf groundtruth:81 track |
...
| `tracking.deepsort(max_age=10, n_init=2, max_iou_distance=0.5)` | PASS | 0.7 | backend=deepsort groundtruth:180 track |
| `tracking overwrite: sort -> jpdaf, get_tracks() tek backend` | PASS | 1.5 | backends=['jpdaf'] |
| `get_tracks(backend='jpdaf') sort varken` | PASS | 0.3 | {} |
| `assessment.evaluate_tracks() GT-sort vs yolo26-sort` | PASS | 3.4 | {} |
| `assessment.evaluate_tracks(match_min_distance_pixel=10, backend='sort')` | PASS | 3.2 | ok |
| `assessment.evaluate_tracks() tek kaynakla (sadece GT)` | PASS | 0.3 | {} |
| `assessment.evaluate_tracks() tracking YOKKEN -> hata beklenir` | **FAIL** | 0.2 | beklenen Exception gelmedi, sonuc: <pycasa.casa.casa.Casa object at 0x00000240BBF31F10> |
...
| `motility [jpdaf/GT]` | PASS | 4.1 | groundtruth: grades={'rapid': 41.51, 'slow': 31.13, 'non_progressive': 2.83, 'immotile': 24.53} motile=75.47 conc=67.76 tracks=106 |
| `motility [deepsort/GT]` | PASS | 2.2 | groundtruth: grades={'rapid': 44.33, 'slow': 29.9, 'non_progressive': 5.15, 'immotile': 20.62} motile=79.38 conc=63.02 tracks=97 |
| `motility [sort/GT+yolo26 iki kaynak]` | PASS | 9.5 | groundtruth: grades={'rapid': 47.52, 'slow': 28.71, 'non_progressive': 2.97, 'immotile': 20.79} motile=79.21 conc=61.21 tracks=101 |
...
| `kinematic_parameters(overlap=1.5) gecersiz -> hata beklenir` | **FAIL** | 1.4 | beklenen Exception gelmedi, sonuc: <pycasa.casa.casa.Casa object at 0x000002477F7B8890> |
...
| `casa_parameters(hyperactivation esikleri, experimental)` | PASS | 1.5 | groundtruth: grades={'rapid': 47.52, 'slow': 28.71, 'non_progressive': 2.97, 'immotile': 20.79} motile=79.21 conc=61.21 tracks=101 |
| `casa_parameters(velocity_metric='XYZ') -> hata beklenir` | PASS | 0.9 | beklenen hata: ValueError: `velocity_metric` must be one of ('VAP', 'VCL', 'VSL'), got 'XYZ'. |
| `casa_parameters() kinematic YOKKEN -> hata beklenir` | PASS | 0.9 | beklenen hata: RuntimeError: No kinematic parameters found. Run `self.motility.kinematic_parameters()` before `casa_parameters()`. |
| `kinematic_parameters() tracking YOKKEN -> hata beklenir` | **FAIL** | 0.4 | beklenen Exception gelmedi, sonuc: <pycasa.casa.casa.Casa object at 0x000002470DB49BB0> |
| `kinematic_parameters() um_per_px=None iken` | **FAIL** | 0.6 | ValueError: `um_per_px` is required when conversion is requested. |
| `get_motility() track basina kinematik anahtarlar` | PASS | 1.3 | ['VCL', 'VSL', 'VAP', 'LIN', 'ALH', 'WOB', 'STR', 'MAD', 'frame_ranges'] |
...
| `visualization.plot_frame('grayscale+binarized')` | PASS | 0.4 | png kaydedildi |
| `visualization.timelapse(tracks+ids+ozel renkler+max_track_gap)` | PASS | 0.7 | png kaydedildi |
| `visualization.interactive_motility_calculator()` | PASS | 1.0 | png kaydedildi |
| `visualization.interactive_motility_calculator(frame_rate=60, smoothing_window=3)` | PASS | 0.9 | png kaydedildi |
| `visualization.plot_frame('moving_cells')` | PASS | 0.2 | png kaydedildi |
| `visualization.timelapse('moving_cells')` | PASS | 0.3 | png kaydedildi |
...
| `visualization.motility_radar()` | PASS | 0.3 | png kaydedildi |
...
| `visualization.motility_density_scatter()` | PASS | 0.8 | png kaydedildi |
