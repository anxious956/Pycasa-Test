...
govde.push(
  ...
  P("YOLO26 returns different results depending on whether YOLOv5 ran earlier in the same Python process. This was narrowed down experimentally: merely importing the YOLOv5 repository modules is enough to change the outcome. OpenCV thread count and the confidence threshold were both ruled out as causes, so the import leaves global state that changes numerical behaviour."),
  P("The obvious candidate was OpenCV's thread count, which the YOLOv5 import drops from the default to one. That was tested directly rather than inferred, with a two-by-two design in which the import and the thread count were varied independently, each condition in its own process."),
  Tablo(["Run", "YOLOv5 imported", "cv2 threads", "Detections", "TP", "FP", "F1"], D.kontrol2x2, [8, 18, 14, 16, 14, 14, 16]),
  P("Varying the thread count on its own changes nothing: A and B are identical, and so are C and D. Varying the import with the thread count held equal does change the result: B against D differs by 387 detections. True positives are 4,120 in all four conditions. Thread count is therefore not the mechanism, and the import is sufficient on its own.", { before: 70 }),
  P("Five further candidates were tested and eliminated, each with a direct experiment rather than an inference:", { before: 70, after: 60 }),
  Madde("Module shadowing. The YOLOv5 repository uses the generic package names models and utils, which enter sys.modules on import. Removing them afterwards does not restore the isolated result."),
  Madde("Environment variables. The import writes OMP_NUM_THREADS, NUMEXPR_MAX_THREADS and three logging variables. Setting all five by hand before importing torch, with no YOLOv5 import, leaves the result unchanged at the isolated value."),
  Madde("Torch global flags. Thread counts, cuDNN benchmark and determinism, TF32 on both paths, float32 matmul precision, default dtype and gradient mode are byte-identical before and after the import."),
  Madde("Non-maximum suppression. torchvision.ops.nms is the same function object before and after, so it has not been monkey-patched."),
  Madde("Confidence threshold. Passing conf explicitly still shifts the result, so the library is not silently substituting a different threshold."),
  P("So the mechanism is still unidentified. The import pulls in 2,629 modules and one of them leaves state that the probes above do not reach; a plausible remaining candidate is a change in DLL search order, since importing the repository prepends an OpenCV binary directory to PATH, but testing that properly is a separate piece of work. The finding should therefore be read as a statement of sufficiency, that importing YOLOv5 is enough to change YOLO26 output, and not as a diagnosed root cause. It is reported because it affects reproducibility regardless of mechanism.", { before: 70, italics: true }),
...
govde.push(
  ...
  Madde("The YOLO26 ordering defect makes benchmark numbers depend on run order. This is the highest-priority fix."),
