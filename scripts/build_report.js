...
govde.push(
  ...
  Tablo(
    ...
    [
      ...
      ["torch.jit.load errno 2 on a valid file", "The working directory path contained a non-ASCII character; the TorchScript loader cannot open such paths", "Keep YOLO weights on an ASCII-only path via PYCASA_PROJECT_ROOT"],
    ],
    [30, 38, 32]),
  P("The last item is worth emphasising. Data loading, YOLO26, moving-cells, digital washing, urbano detection, all three trackers, motility and every visualization work correctly from a non-ASCII path. Only the YOLOv5 TorchScript loader fails. Directory junctions and substituted drives do not help, because pycasa resolves the environment variable with Path.resolve(), which follows the alias back to the real path.", { before: 80 }),
);
...
govde.push(
  ...
  P("The default session loads 101 of 901 frames, which is 3.4 seconds of footage. Concentration and total count are derived from a cells-per-frame average, so they are sensitive to how much footage is available. Steps 5 and 6 were therefore repeated on a " + D.uzun.frames + "-frame window, " + D.uzun.saniye + " seconds, to test that sensitivity."),
  P("The full 899-frame clip could not be loaded on this machine. pycasa holds the video as one contiguous array, which needs 3.3 GB in a single allocation; Windows could not satisfy that request while other work was in memory. This is itself a useful observation about the library's memory model and is listed in section 9.", { italics: true, before: 60 }),
  H("8.1 Tracking over a longer window", HeadingLevel.HEADING_2),
  Tablo(["Tracker", "Tracks (101f)", "Avg. length (101f)", "Tracks (" + D.uzun.frames + "f)", "Avg. length (" + D.uzun.frames + "f)"],
        D.trackingUzun, [20, 20, 20, 20, 20]),
  P("The gap between the two trackers widens sharply with more footage. On 101 frames JPDAF's tracks were 12 percent longer than SORT's; on " + D.uzun.frames + " frames they are " + D.uzun.jpdafKat + " percent longer, and JPDAF consolidates the same cells into " + D.uzun.jpdafAz + " percent fewer tracks. This is exactly the fragmentation problem the 2017 paper set out to solve, and it is only visible once the clip is long enough for tracks to break.", { before: 70 }),
  H("8.2 CASA parameters over a longer window", HeadingLevel.HEADING_2),
...
govde.push(
  ...
  P("Six issues were identified and reproduced. They are ordered by how much they affect results."),
  ...
  P("True positives agree to within three detections across all three runs while false positives span a 47 percent range. Any reported YOLO26 precision or F1 should therefore state the run conditions. The safe practice is to run each detector in a fresh process.", { before: 70 }),
